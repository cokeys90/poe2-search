...
//
// ✅ 조각은 '한 줄 안에서' 매칭돼야 한다 — 인게임으로 확인한 사실이다.
// 게임 검색의 "."은 줄바꿈을 넘지 않는다 (서로 다른 두 모드의 단어로 "정확.*생명"을 넣어 미매칭 확인).
// 따라서 여러 줄에 걸친 조각은 게임에서 절대 안 잡힌다.
// 우리 데이터는 옵션이 차지하는 줄들을 " / "로 이어 적으므로, 검사할 때 다시 줄로 쪼갠다.
