...
import { LANG, LANGS, DEFAULT_LANG, setLang, preloadLang, onLangChange } from "../data/options.js";
...
// 게임 데이터와 화면 문구를 '둘 다' 받아온 뒤에 바꾼다.
// ⚠️ 하나만 바뀌면 옵션은 한국어인데 버튼은 영어인 반쪽 화면이 된다. 실제로 겪었다 —
//    로케일 로딩이 조용히 실패하는데 화면 문구만 바뀌어 있었다. 그래서:
//    ① 게임 데이터를 **먼저 받아 두고**(여기서 실패하면 아무것도 안 바뀐다)
//    ② 문구를 받고 ③ 그때 한꺼번에 바꾼다.
async function apply(lang) {
  await preloadLang(lang); // 실패하면 여기서 throw — 화면은 이전 언어 그대로다
  applyFont(lang); // 폰트는 미리 걸어둔다 — 글자가 그려질 때 이미 요청이 나가 있게
  await loadUi(lang);
  await setLang(lang); // 받아둔 걸 꽂기만 한다 (즉시). 여기서 구독자에게 알림이 간다
...
export function useLang() {
  ...
  const change = useCallback(async (next) => {
    ...
    try {
      ...
      // 반쪽으로 바뀌느니 이전 언어를 그대로 둔다. 저장도 안 한다 (다음에 또 깨지면 안 되니까)
      console.error(`[lang] ${next} 전환 실패 — 이전 언어를 유지한다`, e);
    }
  }, []);
...
export async function restoreLang() {
  ...
  if (!LANGS.includes(lang) || lang === DEFAULT_LANG) {
    syncPath(DEFAULT_LANG);
    return;
  }
  try {
    await apply(lang);
  } catch (e) {
    // 반쪽 화면(옵션은 한국어인데 버튼은 영어)으로 뜨느니 기본 언어로 뜬다
    console.error(`[lang] ${lang} 복원 실패 — 기본 언어로 띄운다`, e);
    syncPath(DEFAULT_LANG);
  }
}
